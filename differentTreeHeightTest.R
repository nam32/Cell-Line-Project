...
#loops through the whole synthetic tree of the fine 
#grained data with varying heights for each level in the
# order to find optimal height for this particular data 
#set. This is validated using H3K27ac3 which was 
#hypothesized to give the best clusters. 3 differently 
#preprocessing of data are used (log2(), log10() and the 
#raw data). The results are then plotted and the heights
# that give the best scores for each type are stored in
#a dataframe.
...
heightsTestNolog <- as.data.frame(setNames(replicate(9,numeric(0), simplify = F), c("mark", "corr", "linkage", "i", "j", "k", "cophenetic value 1", "cophenetic value 2", "baker's gamma value")))
marksVec <- ("H3K27ac3")
corVec <- c("spearman", "pearson")
linkagesVec <- c("complete")
      ...
      for (i in 1:5){
        ...
        for (j in 6:11){
          ...
          for (k in 47:60){
            ...
            cophen_1 <- cor(cophenetic(compareToDend), cophenetic(newdata))
            
            cophen_2 <- cor_cophenetic(compareToDend, newdata)
            
            bakers <- cor_bakers_gamma(compareToDend, newdata)
            
            heightsTestNolog[nrow(heightsTestNolog) + 1, ] <- c(paste0(marks[[x]]), corVec[[y]], linkagesVec[[z]], i, j, k, paste0(cophen_1), paste0(cophen_2), paste0(bakers))
...
heightsTestLog2 <- as.data.frame(setNames(replicate(9,numeric(0), simplify = F), c("mark", "corr", "linkage", "i", "j", "k", "cophenetic value 1", "cophenetic value 2", "baker's gamma value")))
marksVec <- ("H3K27ac3")
corVec <- c("spearman", "pearson")
      ...
      for (i in 1:5){
        ...
        for (j in 6:11){
          ...
          for (k in 47:60){
            ...
            heightsTestLog2[nrow(heightsTestLog2) + 1, ] <- c(paste0(marks[[x]]), corVec[[y]], linkagesVec[[z]], i, j, k, paste0(cophen_1), paste0(cophen_2), paste0(bakers))
...
heightsTestLog10 <- as.data.frame(setNames(replicate(9,numeric(0), simplify = F), c("mark", "corr", "linkage", "i", "j", "k", "cophenetic value 1", "cophenetic value 2", "baker's gamma value")))
marksVec <- ("H3K27ac3")
corVec <- c("spearman", "pearson")
linkagesVec <- c("complete")
      ...
      for (i in 1:5){
        ...
        for (j in 6:11){
          ...
          for (k in 17:22){
            ...
            heightsTestLog10[nrow(heightsTestLog10) + 1, ] <- c(paste0(marks[[x]]), corVec[[y]], linkagesVec[[z]], i, j, k, paste0(cophen_1), paste0(cophen_2), paste0(bakers))
...
cophenetictest <- as.numeric(heightsTestNolog$cophenetic.value.2)
...
plot(nologafter23$cophenetic.value.2, col = factor(nologafter23$X60), cex=2, pch=16)
text(nologafter23$cophenetic.value.2, nologafter23$corr, cex=1, pos = 3, col="red")
...
palette(c("green", "orange"))
plot(nologbefore23$cophenetic.value.2, col = factor(nologbefore23$X22), cex=2, pch=16)
text(nologbefore23$cophenetic.value.2, nologbefore23$corr, cex=1, pos = 3, col="red")
grid(nx = NULL, ny = NULL)
dev.off()
...
cophenetictest <- as.numeric(heightsTestLog2$cophenetic.value.2)
...
#heightsTestLog2Onlyspearman <- heightsTestLog2[grep("spearman",heightsTestLog2$corr, ""), ]
...
palette(c("green", "orange"))
plot(log2after23$cophenetic.value.2, col = factor(log2after23$X60), cex=2, pch=16)
text(log2after23$cophenetic.value.2, log2after23$corr, cex=1, pos = 3, col="red")
grid(nx = NULL, ny = NULL)
dev.off()
...
palette(c("green", "orange"))
plot(log2before23$cophenetic.value.2, col = factor(log2before23$X22), cex=2, pch=16)
text(log2before23$cophenetic.value.2, log2before23$corr, cex=1, pos = 3, col="red")
grid(nx = NULL, ny = NULL)
dev.off()
...
cophenetictest <- as.numeric(heightsTestLog10$cophenetic.value.2)
...
#heightsTestLog10Onlycerfeatures <- heightsTestLog10[grep("spearman",heightsTestLog10$corr, ""), ]
...
palette(c("green", "orange"))
plot(log10after23$cophenetic.value.2, col = factor(log10after23$X60), cex=2, pch=16)
text(log10after23$cophenetic.value.2, log10after23$corr, cex=1, pos = 3, col="red")
grid(nx = NULL, ny = NULL)
dev.off()
...
palette(c("green", "orange"))
plot(log10before23$cophenetic.value.2, col = factor(log10before23$X22), cex=2, pch=16)
text(log10before23$cophenetic.value.2, log10before23$corr, cex=1, pos = 3, col="red")
grid(nx = NULL, ny = NULL)
dev.off()
...
#identifying max value of each data types

#############################
####max of each data type####
#############################

#creating results data frame
optimalVal <- as.data.frame(setNames(replicate(10,numeric(0), simplify = F), c("mark", "corr", "linkage", "i", "j", "k", "cophenetic value 1", "cophenetic value 2", "baker's gamma value", "preprocessing")))

########################################################
#########################no log#########################
########################################################

maxCorrnolog <- max(heightsTestNolog$cophenetic.value.2, na.rm = TRUE)
maxCorrnologdataframe <-heightsTestNolog[grep(paste0(maxCorrnolog), heightsTestNolog$cophenetic.value.2), ]
...
maxCorrLog2 <- max(heightsTestLog2$cophenetic.value.2, na.rm = TRUE)
maxCorrLog2dataframe <- heightsTestLog2[grep(paste0(maxCorrLog2), heightsTestLog2$cophenetic.value.2), ]
...
maxCorrLog10 <- max(heightsTestLog10$cophenetic.value.2, na.rm = TRUE)
maxCorrLog10dataframe <- heightsTestLog10[grep(paste0(maxCorrLog10), heightsTestLog10$cophenetic.value.2), ]
#pearson 0.89145627407426
#spearman 0.909243010430466
nameVec <- as.data.frame(setNames(replicate(1,numeric(0), simplify = F), c("preprocessing")))
